...
let pts t src =
  a_link (sprintf t.Template.pts (escape src)) src

let buildd t show src ver =
  if show
  then a_link (sprintf t.Template.buildd src ver) "build logs"
...
let changelog t ver dir src =
  small [ a_link (sprintf t.Template.changelog dir src ver) ver ]
      ...
      (function
        ...
        | Pcre.Delim s ->
          ...
          if s.[0] = '#' then
            ...
            let link = sprintf tpl.Template.bugs ss in
            a_link link s
          else if s.[0] = '<' then
            let ss = String.sub s 1 (String.length s - 2) in
            let link = sprintf tpl.Template.msg_id ss in
            a_link link s
          else if starts_with l "pts" then
            let text = cut_head s "pts:" in
            let link = sprintf tpl.Template.pts text in
            a_link link s
          else if starts_with l "buildd" then
            let text = cut_head s "buildd:" in
            let link = sprintf tpl.Template.buildd text "" in
            a_link link s
          else
            a_link s s
